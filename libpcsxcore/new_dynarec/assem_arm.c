...
void set_jump_target(int addr,u_int target)
{
  ...
  u_char *ptr_w=(u_char *)to_write_buffer_u8(addr);
  u_int *ptr2_w=(u_int *)to_write_buffer_u32(ptr);
  if(ptr_w[3]==0xe2) {
...
void set_jump_target_fillslot(int addr,u_int target,int copy)
{
  ...
  if(ptr_w[3]==0xe2) {
    ...
    *ptr2_w=(*ptr2_w&0xFFFFF000)|(target-(u_int)ptr2-8);
    ...
    assert((ptr_w[3]&0x0e)==0xa);
    u_int target_insn=*(u_int *)to_write_buffer_u32(target);
...
{
  *((u_int *)to_write_buffer_u32(out))=word;
  out+=4;
}
...
//  u_char *ptr=(u_char *)to_write_buffer_u8(addr);
  ...
  addr = addr + translation_cache_offset;
  u_char *ptr=(u_char *)addr;
