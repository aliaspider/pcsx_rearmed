...
//char* translation_cache_w = translation_cache;
#endif
...
check_translation_cache();
  ...
  u_char *ptr_w=(u_char *)(addr + BASE_ADDR_OFFSET);
  u_int *ptr2_w=(u_int *)(ptr + BASE_ADDR_OFFSET);

  if(ptr_w[3]==0xe2) {
    assert((target-(u_int)ptr2-8)<1024);
    assert((addr&3)==0);
    assert((target&3)==0);
    *ptr2_w=(*ptr2_w&0xFFFFF000)|((target-(u_int)ptr2-8)>>2)|0xF00;
    //printf("target=%x addr=%x insn=%x\n",target,addr,*ptr2);
  }
  else if(ptr_w[3]==0x72) {
    ...
    if((target-(u_int)ptr2-8)<1024) {
      ...
      *ptr2_w=(*ptr2_w&0xFFFFF000)|((target-(u_int)ptr2-8)>>2)|0xF00;
      ...
      assert((addr&3)==0);
      assert((target&3)==0);
      *ptr2_w=(*ptr2_w&0xFFFFF000)|((target-(u_int)ptr2-8)>>4)|0xE00;
    }
    else *ptr2_w=(0x7A000000)|(((target-(u_int)ptr2-8)<<6)>>8);
  }
  else {
    assert((ptr_w[3]&0x0e)==0xa);
    *ptr2_w=(*ptr2_w&0xFF000000)|(((target-(u_int)ptr2-8)<<6)>>8);
  }
  check_translation_cache();
}
...
void set_jump_target_fillslot(int addr,u_int target,int copy)
{
  ...
  u_char *ptr_w=(u_char *)(addr + BASE_ADDR_OFFSET);
  u_int *ptr2_w=(u_int *)(ptr + BASE_ADDR_OFFSET);
  assert(!copy||ptr2_w[-1]==0xe28dd000);
  if(ptr_w[3]==0xe2) {
    assert(!copy);
    assert((target-(u_int)ptr2-8)<4096);
    *ptr2_w=(*ptr2_w&0xFFFFF000)|(target-(u_int)ptr2-8);
  }
  else {
    assert((ptr_w[3]&0x0e)==0xa);
    u_int target_insn=*(u_int *)(target + BASE_ADDR_OFFSET);
...
{   
//  *((u_int *)out)=word;
  *((u_int *)((u32)out + BASE_ADDR_OFFSET))=word;
  out+=4;
}
...
void literal_pool(int n)
{
  ...
  for(i=0;i<literalcount;i++)
  {
    ...
    assert(offset<4096);
    assert(!(offset&3));
    *((u_int *)((u32)ptr + BASE_ADDR_OFFSET))|=offset;
    if(l_addr==(u_int)out) {
